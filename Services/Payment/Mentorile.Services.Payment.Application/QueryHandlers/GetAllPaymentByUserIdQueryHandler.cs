...
public class GetAllPaymentByUserIdQueryHandler : IRequestHandler<GetAllPaymentByUserIdQuery, Result<PagedResult<PaymentDTO>>>
{
    private readonly IPaymentRepository _paymentRepository;
    private readonly IMapper _mapper;

    public GetAllPaymentByUserIdQueryHandler(IPaymentRepository paymentRepository, IMapper mapper)
    {
        _paymentRepository = paymentRepository;
        _mapper = mapper;
    }

    public async Task<Result<PagedResult<PaymentDTO>>> Handle(GetAllPaymentByUserIdQuery request, CancellationToken cancellationToken)
    {
        var payments = await _paymentRepository.GetAllByUserIdAsync(request.UserId, request.PagingParams);
