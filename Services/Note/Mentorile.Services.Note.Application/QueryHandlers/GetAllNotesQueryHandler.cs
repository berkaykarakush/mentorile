...
public class GetAllNotesQueryHandler : IRequestHandler<GetAllNotesQuery, Result<PagedResult<NoteDTO>>>
{
    ...
    public async Task<Result<PagedResult<NoteDTO>>> Handle(GetAllNotesQuery request, CancellationToken cancellationToken)
    {
        var pagingParams = request.PagingParams ?? new PagingParams();
        var notesResult = await _noteRepository.GetAllNotesAsync(pagingParams, cancellationToken);
