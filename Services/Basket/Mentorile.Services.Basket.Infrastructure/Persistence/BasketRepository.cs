...
namespace Mentorile.Services.Basket.Infrastructure.Persistence;
public class BasketRepository : IBasketRepository
{
    private readonly IRedisService _redisService;
    private readonly ISharedIdentityService _sharedIdentityService;

    public BasketRepository(IRedisService redisService, ISharedIdentityService sharedIdentityService)
    {
    ...
    private async Task<(string basketKey, Domain.Entities.Basket basket)> GetOrCreateBasketAsync()
    {
        ...
        {
            basket = new Domain.Entities.Basket { Id = basketId };
        }
        else
        {
            basket = JsonSerializer.Deserialize<Domain.Entities.Basket>(redisResult.Data) ?? new Domain.Entities.Basket { Id = basketId };
        }

        return (basketKey, basket);
    }

    public async Task<Result<bool>> AddItemToBasketAsync(BasketItem item)
    {
   ...
   public async Task<Result<bool>> ApplyDiscountAsync(string discountCode)
    ...
    public async Task<Result<Domain.Entities.Basket>> GetBasketAsync()
    {
        var (_, basket) = await GetOrCreateBasketAsync();
        return Result<Domain.Entities.Basket>.Success(basket);
    }
